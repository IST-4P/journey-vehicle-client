...
const SUPPORT_USER_ID =
  import.meta.env.VITE_SUPPORT_USER_ID ??
  "4d991046-125d-4ed3-a5d7-b3f101768e4e";
...
export function ChatWidget() {
  ...
  const [chatSocket, setChatSocket] = useState<ReturnType<
  ...
  const mapServerMessage = useCallback(
    (msg: ServerChatMessage): ChatMessage => {
  ...
  const handleSendMessage = (content: string) => {
    ...
    if (socket && "emit" in socket && typeof socket.emit === "function") {
      ...
      console.error(
        "[ChatWidget] Socket is not available or does not support emit"
      );
    }
  };
  ...
  useEffect(() => {
    ...
    const offNewChat = socketClient.on(
      "newChat",
      (payload: ServerChatMessage) => handleIncomingMessage(payload)
    );
                ...
                {stage === "intro" ? (
                  <div className="flex flex-1 flex-col items-center justify-center space-y-4 px-6 text-center">
                    <p className="text-sm font-medium text-gray-800 leading-relaxed">
                      Nhấn "Tôi cần hỗ trợ" để kết nối với đội ngũ chăm sóc
                      khách hàng.
                    </p>
                    <p className="text-xs text-gray-500">
                      Chúng tôi sẽ kiểm tra phiên đăng nhập của bạn trước khi mở
                      chat.
                    </p>
                    <Button
