...
import { 
  Package2, Star, Heart, Share2, Calendar, ArrowLeft, 
  ChevronLeft, ChevronRight, ZoomIn, Shield, Check
} from 'lucide-react';
...
export function ComboDetail() {
  ...
  const reviews: Review[] = [
    ...
    },
    {
      id: '2',
      userName: 'Lê Thị E',
      userAvatar: 'https://images.unsplash.com/photo-1494790108377-be9c29b29330?w=64&h=64&fit=crop&crop=face',
      rating: 5,
  ...
  const fetchComboDetail = async () => {
    ...
    try {
      ...
      const mockCombo: Combo = {
        ...
        description: 'Bộ thiết bị đầy đủ cho chuyến camping gia đình 4 người. Combo này bao gồm tất cả những gì bạn cần cho một chuyến cắm trại hoàn hảo, từ lều, túi ngủ đến dụng cụ nấu ăn và chiếu sáng.',
        pricePerHour: 80000,
        images: [
        ...
        devices: [
          { name: 'Lều cắm trại 4 người', quantity: 1, description: 'Lều chống nước Coleman, kích thước 2.1m x 2.1m' },
          { name: 'Túi ngủ', quantity: 4, description: 'Túi ngủ giữ ấm, phù hợp nhiệt độ 5-25°C' },
          { name: 'Bếp gas mini', quantity: 1, description: 'Bếp gas Kovea công suất 2800W' },
          { name: 'Bộ nồi niêu', quantity: 1, description: 'Bộ 5 món chất liệu nhôm anodized' },
          { name: 'Đèn pin LED', quantity: 2, description: 'Đèn pin siêu sáng 300 lumen, pin sạc' },
          { name: 'Bình nước giữ nhiệt', quantity: 2, description: 'Bình Thermos 1L giữ nhiệt 24h' },
          { name: 'Bạt trải sàn', quantity: 1, description: 'Bạt chống thấm kích thước 2.5m x 2.5m' }
        ],
        rating: 4.9,
        reviewCount: 45
      };
  ...
  const prevImage = () => {
    if (combo?.images) {
      setCurrentImageIndex((prev) => (prev - 1 + combo.images!.length) % combo.images!.length);
    }
  };
      ...
      <div className="bg-white border-b">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex items-center space-x-2 text-sm">
            ...
            <Link to="/combos" className="text-gray-600 hover:text-purple-600">Combo</Link>
            <span className="text-gray-400">/</span>
              ...
              <CardContent className="p-6">
                <Tabs defaultValue="description">
                  <TabsList className="w-full">
                    ...
                    <TabsTrigger value="devices" className="flex-1">Thiết bị ({combo.devices.length})</TabsTrigger>
                    <TabsTrigger value="reviews" className="flex-1">Đánh giá ({reviews.length})</TabsTrigger>
                  </TabsList>

                  <TabsContent value="description" className="mt-4">
                    <p className="text-gray-700 leading-relaxed mb-4">{combo.description}</p>
                    <div className="bg-purple-50 border border-purple-200 rounded-lg p-4 mt-4">
                      <h4 className="font-semibold text-purple-900 mb-2">Ưu điểm của Combo</h4>
                  ...
                  <TabsContent value="devices" className="mt-4">
                    <div className="space-y-4">
                      {combo.devices.map((device, idx) => (
                        <div key={idx} className="border rounded-lg p-4 hover:border-purple-300 transition-colors">
                          <div className="flex items-start justify-between mb-2">
                            ...
                            <Badge variant="outline" className="ml-2">x{device.quantity}</Badge>
                          </div>
                        </div>
                      ))}
                    </div>
              ...
              <CardHeader>
                ...
                <div className="flex items-center mt-2">
                  <div className="flex items-center">
                    ...
                    <span className="ml-1 text-sm text-gray-500">({combo.reviewCount} đánh giá)</span>
                  </div>
                </div>
              ...
              <CardContent className="space-y-4">
                {rentalDuration && (
                  <div className="bg-purple-50 border border-purple-200 rounded-lg p-3">
                    <div className="flex items-center space-x-2 mb-2">
                      ...
                      <span className="text-sm font-semibold text-purple-900">Thời gian thuê</span>
                    </div>
                ...
                <div className="space-y-3">
                  <div className="flex justify-between text-sm">
                    ...
                    <span className="font-semibold">{combo.pricePerHour.toLocaleString('vi-VN')}đ</span>
                  </div>
                ...
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
                  <div className="flex items-start space-x-2">
                    ...
                    <div className="text-sm text-blue-900">
                      ...
                      <p className="text-blue-700">Combo giúp bạn tiết kiệm chi phí so với thuê từng thiết bị riêng lẻ.</p>
                    </div>
                  </div>
                </div>
              </CardContent>
